...
(setq reftex-default-bibliography
      '("/Users/lotuc/Documents/bibliobase/bibliobase.bib"))
...
(setq org-refile-targets (quote ((nil :maxlevel . 2)
                                 (org-agenda-files :maxlevel . 2))))
                                        ; Stop using paths for refile targets -
                                        ; we file directly with IDO
(setq org-refile-use-outline-path t)
(setq org-refile-allow-creating-parent-nodes (quote confirm))
                                        ; Allow refile to create parent
                                        ; tasks with confirmation
(add-to-list 'exec-path "/usr/local/bin/")

;; comment/uncomment the two lines below to
;; turn on/off auto fill and turn off/on visual line mode
...
(defun bibretrieve-scholar-create-url (author title)
  (let ((tempfile (make-temp-file "scholar" nil ".bib")))
    (call-process-shell-command
     "~/bin/gscholar/gscholar/gscholar.py --all" nil nil nil
